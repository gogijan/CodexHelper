...
Publish a self-contained single-file win-x64 build:

```powershell
dotnet publish .\CodexHelper\CodexHelper.csproj -p:PublishProfile=win-x64-singlefile
```

The single executable is written to `artifacts\publish\win-x64-singlefile`. This is the largest build, but it includes the .NET runtime and can run on machines without .NET installed.

Publish a smaller single-file win-x64 build that requires the .NET 10 Desktop Runtime on the target machine:
...
- Publish profiles are in `CodexHelper/Properties/PublishProfiles`.
...
Опубликуй self-contained сборку одним exe-файлом:

```powershell
dotnet publish .\CodexHelper\CodexHelper.csproj -p:PublishProfile=win-x64-singlefile
```

Один исполняемый файл будет в `artifacts\publish\win-x64-singlefile`. Это самый большой вариант, зато он включает .NET runtime и запускается на машинах без установленного .NET.
...
- Publish profiles находятся в `CodexHelper/Properties/PublishProfiles`.
